...
def create_default_tagger() -> ConversationTagger:
    ...
    # Rule to detect programming languages mentioned
    def mentions_programming_language(exchange):
        """Detect programming language mentions."""
        text = (exchange.get_user_text() + ' ' + exchange.get_assistant_text()).lower()
        languages = ['python', 'javascript', 'java', 'c++', 'rust', 'go', 'sql']
        mentioned = [lang for lang in languages if lang in text]
        if mentioned:
            return Tag('mentions_programming_language', languages=mentioned)
        return False
